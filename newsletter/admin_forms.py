...
class ImportForm(forms.Form):
    def clean(self):
        ...
        if not (
            ...
            and "newsletter" in self.cleaned_data
        ):
    ...
    ignore_errors = forms.BooleanField(
        label=_("Ignore non-fatal errors"), initial=False, required=False
    )
...
class ConfirmForm(forms.Form):
    ...
    confirm = forms.BooleanField(
        label=_("Confirm import"), initial=True, widget=forms.HiddenInput
    )
...
class SubscriptionAdminForm(forms.ModelForm):
    ...
    def clean_email_field(self):
        ...
        if self.cleaned_data["user"] and data:
            raise forms.ValidationError(
                _("If a user has been selected this field " "should remain empty.")
            )
        return data
    ...
    def clean(self):
        ...
        if not (
            cleaned_data.get("user", None) or cleaned_data.get("email_field", None)
        ):
            raise forms.ValidationError(
                _(
